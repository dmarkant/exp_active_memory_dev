...
@custom_code.route('/manifest')
def manifest():
    """Taken from:
    http://stackoverflow.com/questions/17351880/how-to-build-an-offline-web-app-using-flask
    """
    res = make_response(render_template('appcache.manifest'), 200)
...
def setup():
    ...
    for partner in q:
        ...
        # don't include any ids from retesting
        if pid.count('-retest') == 0 and pid.count('throwaway') == 0:
